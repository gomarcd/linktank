    ...
    @foreach ($this->getBookmarks as $bookmark)
        <div class="flex justify-between items-start p-4 border border-accent rounded-lg mb-4">
            <div class="w-4/5 ml-1">
                ...
                <button type="button" x-data="{ input: '{{ $bookmark->url }}' }" x-clipboard="input" @click="Toaster.success('Copied to clipboard')" class="hover:text-success">
            ...
            <div>
                ...
                <!-- Delete Button -->
                <button type="button" wire:click="delete({{ $bookmark->id }})" class="hover:text-red-600">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
