<div role="status" id="toaster" x-data="toasterHub(@js($toasts), @js($config))" @class([
    ...
    <template x-for="toast in toasts" :key="toast.id">
        ...
        >
            ...
            <!-- Original - only allows plain text -->
            ...
            <!-- New - allows inserting HTML, added to include undo button in toast message -->
            <i x-html="toast.message"
                class="block select-none not-italic px-6 py-3 rounded shadow-lg text-sm w-full {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }}"
                :class="toast.select({ error: 'bg-red-500', info: 'bg-gray-200', success: 'bg-green-600', warning: 'bg-orange-500' })">
            </i>
